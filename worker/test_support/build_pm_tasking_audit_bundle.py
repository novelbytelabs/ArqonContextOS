...
FILES = [
  ...
  "worker/src/index.ts", "worker/src/pm_handoff.ts", "worker/src/pm_intake.ts", "worker/src/pm_specify.ts", "worker/src/pm_plan.ts", "worker/src/pm_tasking.ts",
...
def main() -> None:
    ...
    manifest = {"schema_version": "pm_tasking_audit_bundle.v0.1", "commit": commit, "required_status_labels": LABELS, "files": records}
    out = root / "temps" / f"pm_tasking_audit_bundle_{commit[:12]}.zip"
