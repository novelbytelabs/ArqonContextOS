...
FILES = [
  ...
  "worker/test_support/code_monkeys_human_advancement_decision_policy_unit.ts",
  "worker/test_support/code_monkeys_human_advancement_decision_offline_smoke.ts",
  "worker/test_support/code_monkeys_human_advancement_decision_live_smoke.ts",
  "worker/test_support/code_monkeys_human_advancement_decision_tripwire.py",
  ...
  "docs/04_flows_and_spec_kit/HUMAN_ADVANCEMENT_GATE_001_EVIDENCE.md",
]
...
def main():
  ...
  out = root/"temps"/f"human_advancement_decision_audit_bundle_{commit[:12]}.zip"; out.parent.mkdir(exist_ok=True)
  records = [{"path":p,"bytes":(root/p).stat().st_size,"sha256":sha(root/p)} for p in FILES]
  manifest = {"schema_version":"human_advancement_decision_audit_bundle.v0.1","commit":commit,"purpose":"verify Human Advancement Gate 001 decision boundary","required_status_labels":LABELS,"files":records}
