...
async function main(): Promise<void> {
  ...
  console.log(JSON.stringify({ ok:true, no_auth_status:noAuth.status, success_status:success.status, replay_status:replay.status, conflict_status:conflict.status, bad_title_status:badTitle.status, bad_summary_status:badSummary.status, sha_status:sha.status, wrote_artifacts:writes.filter(p=>p.includes("/artifacts/")).length }, null, 2));
}
